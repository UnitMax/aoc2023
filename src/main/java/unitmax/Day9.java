...
public class Day9 {
    ...
    public static long extrapolateBackwards(List<Long> numbers) {
        boolean allZeroes = false;
        List<List<Long>> loloDiffs = new ArrayList<>();
        List<Long> orig = numbers;
        while (!allZeroes) {
        ...
        for (int i = 0; i < (loloDiffs.size() - 1); i++) {
            var current = loloDiffs.get(i);
            var next = loloDiffs.get(i + 1);
            long newVal = next.get(0)/* y */ - current.get(0) /* d */;
            next.add(0, newVal);
        }
        var finalVal = orig.get(0) - loloDiffs.get(loloDiffs.size() - 1).get(0);
        return finalVal;
    }

    public static long sumOfExtrapolatedValues(String[] input) {
        long ctr = 0;
        for (String s : input) {
    ...
    public static long sumOfExtrapolatedValuesBackwards(String[] input) {
        long ctr = 0;
        for (String s : input) {
            ctr += extrapolateBackwards(Arrays.stream(s.split("\s")).map(String::strip).map(Long::parseLong).collect(Collectors.toList()));
        }
        return ctr;
    }
}
